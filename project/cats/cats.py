...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    ans = [c for c in paragraphs if select(c)]
    if len(ans) > k:
        return ans[k]
    return ""
    # END PROBLEM 1
...
def about(subject):
    ...
    def select_func(para):
        ans = str(remove_punctuation(para)).lower().split()
        for c in ans:
            if c in subject:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        minimum = 10000
        for c in word_list:
            if diff_function(typed_word, c, limit) < minimum:
                minimum = diff_function(typed_word, c, limit)
                final_word = c
        if minimum > limit:
            return typed_word
        else:
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_word_list = [[] for _ in player_indices]
    for i in word_indices:
        mini = 100
        for j in player_indices:
            if time(match, j, i) < mini:
                mini = time(match, j, i)
                mini_index = i
                mini_player = j
        fastest_word_list[mini_player].append(get_word(match, mini_index))
